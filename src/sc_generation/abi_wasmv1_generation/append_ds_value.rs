...
pub fn generate_abi_append_ds_value(calls: &mut Vec<String>, preparation_calls: &mut Vec<String>) {
    let mut rng = rand::thread_rng();

    let rng_key_bytes = generate_bytes(rng.gen_range(10..64));
    let rng_value1_bytes = generate_bytes(rng.gen_range(1..1000));
    let rng_value2_bytes = generate_bytes(rng.gen_range(1..1000));
    ...
    calls.push(format!(
        "{{let key_bytes = new Uint8Array({});
        key_bytes.set({:?});
        let value2_bytes = new Uint8Array({});
        value2_bytes.set({:?});
        env.append_ds_value(key_bytes, value2_bytes, null);}}",
        rng_key_bytes.len(),
        rng_key_bytes,
        rng_value2_bytes.len(),
        rng_value2_bytes,
    ));
}
